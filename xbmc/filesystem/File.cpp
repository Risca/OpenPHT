...
bool CFile::Open(const CStdString& strFileName, unsigned int flags, size_t cacheSize)
{
  ...
  try
  {
    ...
    catch (CRedirectException *pRedirectEx)
    {
      ...
      {
        boost::scoped_ptr<CURL> pNewUrl(pRedirectEx->m_pNewUrl);
...
bool CFile::Exists(const CStdString& strFileName, bool bUseCache /* = true */)
{
  ...
  try
  {
    ...
    boost::scoped_ptr<IFile> pFile(CFileFactory::CreateLoader(url));
    if (!pFile.get())
      return false;
  ...
  catch (CRedirectException *pRedirectEx)
  {
    ...
    if (pRedirectEx && pRedirectEx->m_pNewFileImp)
    {
      boost::scoped_ptr<IFile> pImp(pRedirectEx->m_pNewFileImp);
      boost::scoped_ptr<CURL> pNewUrl(pRedirectEx->m_pNewUrl);
      delete pRedirectEx;
